...
document.addEventListener("DOMContentLoaded", function () {
  ...
  data.forEach((item) => {
    ...
    salesByMonth[key][item.dealer] += item.profit;
  });
  ...
  headerCell1.textContent = "Total Profit";
...
function createData(dealerData, tableBody) {
  Object.entries(dealerData)
    .sort(([, profitA], [, profitB]) => profitB - profitA)
    .forEach(([dealer, totalProfit]) => {
      let row = document.createElement("tr");
      let cell0 = document.createElement("td");
      let cell1 = document.createElement("td");

      cell0.textContent = dealer;
      cell1.textContent = totalProfit;
